...
function setUserColors(clr) {
  if (isPlaying) {
    if (userColors.length < 4) {
      userColors.push(clr);
      const gridPos = document.getElementById(
        String(tries) + (userColors.length - 1)
      );
    ...
    <>
      <div className="color-div">
        ...
        <center>
          <Button
            colorScheme="red"
            className="R color-btn select-button"
            onClick={() => setUserColors("R")}
          />
          <Button
            colorScheme="green"
            className="G color-btn select-button"
            onClick={() => setUserColors("G")}
          />
          <Button
            colorScheme="blue"
            className="B color-btn select-button"
            onClick={() => setUserColors("B")}
          />
          <br />
          <Button
            colorScheme="yellow"
            className="Y color-btn select-button"
            onClick={() => setUserColors("Y")}
          />
          <Button
            colorScheme="purple"
            className="P color-btn select-button"
            onClick={() => setUserColors("P")}
          />
          <Button
            colorScheme="orange"
            className="O color-btn select-button"
            onClick={() => setUserColors("O")}
          />
          <br />
        </center>
      </div>
      ...
      <br />
      <div className="ans-div">{answer()}</div>
    </>
  );
}
